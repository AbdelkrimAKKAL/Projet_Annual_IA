...
EXPORT LinearModel* create_linear_model(int input_size, int output_size) {
    ...
    // Initialisation aleatoire (seed fixe pour pouvoir comparer les runs)
    srand(42);
...
// 2- Destruction propre de la memoire
...
// 3- L'Entrainement sur une seule image
EXPORT void train_one_linear(LinearModel* m, double* features, int label, double learning_rate) {
    ...
    // Calcul de l'erreur et mise a jour
    for (int i = 0; i < m->output_size; i++) {
...
// 4bis- Entrainement pour la regression (sortie reelle, pas de argmax).
// Suppose output_size == 1 : un seul neurone de sortie, activation lineaire (identite).
EXPORT void train_one_linear_regression(LinearModel* m, double* features, double target, double learning_rate) {
    double score = m->biases[0];
    for (int j = 0; j < m->input_size; j++) {
        score += m->weights[0][j] * features[j];
    }

    double error = target - score;
    m->biases[0] += learning_rate * error;
    for (int j = 0; j < m->input_size; j++) {
        m->weights[0][j] += learning_rate * error * features[j];
    }
}

// 4ter- Prediction pour la regression : renvoie la valeur reelle, pas une classe
EXPORT double predict_linear_regression(LinearModel* m, double* features) {
    double score = m->biases[0];
    for (int j = 0; j < m->input_size; j++) {
        score += m->weights[0][j] * features[j];
    }
