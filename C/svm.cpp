...
EXPORT void train_one_svm(SVM* m, double* features, double* ybin, int nbr_ex, int classe){
    ...
    double** bigM = (double**)malloc(nbr_ex * sizeof(double*));
    for (int i=0; i<nbr_ex; i++){
        bigM[i] = (double*)malloc(nbr_ex * sizeof(double));
    ...
    for (int i=0; i<nbr_ex; i++){
        if (alpha[i] > 1e-6){
            cm->sv_features[ct] = (double*)malloc(m->input_size * sizeof(double));
...
EXPORT void train_svm(SVM* m, double* features, double* labels, int nbr_ex){
    ...
    double* ybin = (double*)malloc(nbr_ex * sizeof(double));
...
// 6- Prediction : f(x) = w0 + somme_k( alpha_k * y_k * K(x_k, x) )
...
EXPORT SVM* load_svm(const char* path){
    ...
    for (int c = 0; c < output_size; c++){
        ...
        cm->sv_features = (double**)malloc(cm->nb_sv * sizeof(double*));
        cm->sv_alpha = (double*)malloc(cm->nb_sv * sizeof(double));
        cm->sv_y = (double*)malloc(cm->nb_sv * sizeof(double));

        for (int i = 0; i < cm->nb_sv; i++){
            fscanf(f, "%lf %lf", &cm->sv_alpha[i], &cm->sv_y[i]);
            cm->sv_features[i] = (double*)malloc(input_size * sizeof(double));
