...
lib.create_svm_model.argtypes = [c_int, c_int]
...
def entrainer_svm(X, y, nb_classes=None):
    """Cree et entraine un modele SVM (one-vs-all, lineaire) sur (X, y)"""
    X_np = np.ascontiguousarray(X, dtype=np.float64)
    y_np = np.ascontiguousarray(y, dtype=np.float64)
    if nb_classes is None:
        nb_classes = len(set(y))

    model = lib.create_svm_model(X_np.shape[1], nb_classes)
    lib.train_svm(model,
